<?php
require_once __DIR__ . '/mail_shared.php';

$p = getPersonalFields();

// ── AIET-specific programme fields ──
$progType = clean($_POST['aiet_progType'] ?? '');
$course   = clean($_POST['aiet_course']   ?? '');
$year     = clean($_POST['aiet_year']     ?? '');
$sem      = clean($_POST['aiet_sem']      ?? '');
$mode     = clean($_POST['aiet_mode']     ?? '');
$referral = clean($_POST['aiet_referral'] ?? '');
$ref      = $p['ref'];

if (!$p['firstName'] || !$p['lastName'] || !$p['email'] || !$progType || !$ref) {
    echo json_encode(['success' => false, 'message' => 'Missing required fields.']);
    exit;
}

// ── Uploads ──
$uploadDir = __DIR__ . '/uploads/applications/';
if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
$uploads = processUploads($uploadDir, $ref);

// ── Build email body ──
$nl      = "\r\n";
$now     = date('l, d F Y \a\t H:i T');
$subject = "New Application | AIET | {$progType} — {$course} | {$p['firstName']} {$p['lastName']} [{$ref}]";

$body  = "AIET — AMERICAN INSTITUTE OF EDUCATION & TRAINING{$nl}";
$body .= "ONLINE APPLICATION{$nl}";
$body .= "================================================={$nl}";
$body .= "Reference Number : {$ref}{$nl}";
$body .= "Submitted On     : {$now}{$nl}{$nl}{$nl}";

$body .= "[ 1 ] PROGRAMME SELECTION{$nl}";
$body .= "--------------------------{$nl}";
$body .= "Programme Type   : {$progType}{$nl}";
$body .= "Course           : " . ($course ?: 'Not specified') . "{$nl}";
$body .= "Intake Year      : {$year}{$nl}";
$body .= "Intake / Semester: {$sem}{$nl}";
$body .= "Study Mode       : {$mode}{$nl}";
$body .= "Referral Source  : " . ($referral ?: 'Not provided') . "{$nl}{$nl}{$nl}";

$body .= buildPersonalSection($p, $nl);
$body .= buildAcademicSection($p, $nl);
$body .= buildProfessionalSection($p, $nl);
$body .= buildReferenceSection($p, $nl);
$body .= buildUploadsSection($uploads, $GLOBALS['uploadLabels'], $nl);

$body .= "{$nl}================================================={$nl}";
$body .= "Files saved on server : uploads/applications/{$nl}";
$body .= "Contact applicant     : {$p['email']}{$nl}";
$body .= "Reference             : {$ref}{$nl}";

// ── Send ──
$result = sendEmail(
    'noreply@aietglobal.us', 'AIET Admissions Portal',
    'info@aietglobal.us',    'AIET Admissions',
    $p['email'], trim("{$p['firstName']} {$p['lastName']}"),
    $subject, $body, $uploads
);

echo json_encode([
    'success' => $result['sent'],
    'ref'     => $ref,
    'warning' => $result['warning'] ?: null,
]);
